  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      setError(error.message === 'Invalid login credentials'
        ? (isEn ? 'Incorrect email or password' : 'Email atau password salah')
        : error.message)
      setLoading(false)
    } else {
      navigate('/')
      ...
      >
        <div className="bg-white/95 backdrop-blur-xl rounded-[2rem] shadow-2xl p-8 border border-white/50">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-fredoka font-bold text-gray-900 mb-2">{isEn ? 'Welcome Back!' : 'Selamat Datang!'}</h1>
            <p className="text-gray-500 font-outfit text-sm">{isEn ? 'Sign in to your Robo Mind account' : 'Masuk ke akun Robo Mind Anda'}</p>
          </div>
          ...
          <p className="text-center text-sm text-gray-500 font-outfit mt-6">
            ...
            <Link to="/register" className="text-primary-600 font-bold hover:underline">{isEn ? 'Sign Up' : 'Daftar'}</Link>
          </p>
        </div>

        <p className="text-center text-xs text-gray-400 font-outfit mt-6">
          {isEn ? 'By signing in, you agree to our' : 'Dengan masuk, Anda menyetujui'}{' '}
