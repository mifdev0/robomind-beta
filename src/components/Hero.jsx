...
const Hero = () => {
  ...
  const slides = [
    ...
    {
      id: 1,
      tag: i18n.language === 'en' ? 'CLASS DOCUMENTATION' : 'DOKUMENTASI KELAS',
      title: i18n.language === 'en' ? 'See RoboMind Learning in Action' : 'Lihat Kegiatan Kelas RoboMind Secara Langsung',
      subtitle: i18n.language === 'en'
        ? 'A real classroom moment showing how children learn, play, and build focus with RoboMind.'
        : 'Momen kelas nyata saat anak belajar, bermain, dan melatih fokus bersama RoboMind.',
    ...
    },
    {
      ...
      subtitle: i18n.language === 'en'
        ? "Connect your child's game, monitor development metrics in real-time, and get specific recommendations from child psychologists."
    ...
    },
    {
      ...
      subtitle: i18n.language === 'en'
        ? "Play while honing coding logic. Let your little one's imagination fly high without limits."
        : 'Bermain sambil mengasah logika coding. Biarkan imajinasi si kecil terbang tinggi tanpa batas.',
      img: '/assets/images/hero_classroom.png',
    },
    {
      id: 4,
      tag: i18n.language === 'en' ? 'REAL-TIME MONITORING' : 'PANTAUAN REAL-TIME',
      title: i18n.language === 'en' ? 'See Development Directly' : 'Lihat Perkembangan Secara Langsung',
      subtitle: i18n.language === 'en'
        ? 'Access an exclusive dashboard to track language competence, focus, and problem solving.'
        : 'Akses dashboard eksklusif untuk melacak kompetensi bahasa, fokus, dan pemecahan masalah.',
    ...
    <section id="beranda" className="relative w-full h-[85vh] min-h-[580px] sm:h-[70vh] md:h-[60vh] lg:h-[600px] bg-black overflow-hidden group">
      ...
      <div className="absolute bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 md:left-auto md:translate-x-0 md:right-8 z-20 flex items-center gap-2 md:gap-6">
        ...
        <div className="flex items-center gap-1 md:gap-2">
          <button className="custom-swiper-prev w-6 h-6 md:w-10 md:h-10 rounded-full bg-black/40 backdrop-blur-md border border-white/20 flex items-center justify-center text-white hover:bg-white hover:text-black transition-colors cursor-pointer" aria-label="Previous slide">
            <ChevronLeft size={12} className="md:size-[16]" />
          </button>
          <button className="custom-swiper-next w-6 h-6 md:w-10 md:h-10 rounded-full bg-black/40 backdrop-blur-md border border-white/20 flex items-center justify-center text-white hover:bg-white hover:text-black transition-colors cursor-pointer" aria-label="Next slide">
